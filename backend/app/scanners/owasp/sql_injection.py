    ...
    async def scan(self, endpoints: List[str]) -> List[Vulnerability]:
        ...
        for idx, endpoint in enumerate(endpoints, 1):
            if self.progress_callback:
                await self.progress_callback(f"[SQLI] Testing endpoint {idx}/{total_endpoints}: {endpoint[:60]}...")
            ...
            try:
                ...
                if vulns and self.progress_callback:
                    await self.progress_callback(f"[+] Found {len(vulns)} SQLi on {endpoint[:60]}")
